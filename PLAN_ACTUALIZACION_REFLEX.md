...
|---|---|---|---|---|
| 1 | 2026-08-09 | Food | `pytest`: `test_bloqueo_empresa_suspendida_o_inexistente` falla. `evaluar_bloqueo(None,...)` devuelve `"Empresa no encontrada — contacte soporte."` pero el test espera `MSG_SUSPENDIDA`. **PREEXISTENTE, ajeno a Reflex** (archivo `suscripcion_service.py` no se tocó). | Pendiente decidir: ¿unificar mensaje o actualizar el test? NO bloquea la actualización. |
